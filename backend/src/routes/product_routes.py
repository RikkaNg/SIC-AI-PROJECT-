...
def get_top_products(
    ...
    try:
        ...
        rows = conn.execute(f"""
            ...
            {scope_sql}
            GROUP BY c.item_nbr, i.family, i.class, i.perishable
...
def get_family_trend(
    ...
    try:
        ...
        if has_agg:
            ...
            """, [days, *agg_params, top_families]).fetchall()
            ...
            series_rows = conn.execute(f"""
                ...
                ORDER BY a.date
            """, [days, *agg_params, *families]).fetchall()
        ...
        """, [days, *params, top_families]).fetchall()
        families = [r["family"] for r in top_rows]
